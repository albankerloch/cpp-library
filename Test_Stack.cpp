...
template <typename T>
bool CompareStack(ft::stack<T> *ft, std::stack<T> *stl, const std::string &testName) 
{
	typename std::stack<T>::iterator it_stl;
	typename ft::stack<T>::iterator it;
	size_t i;
...
void ft_capacity()
{
	std::stack<int> V1STL (3, 39);
	ft::stack<int> V1 (3, 39);
	std::stack<int> V2STL;
	ft::stack<int> V2;
	/*
	size
...
reserve*/
	...
	if (V1.capacity() == V1STL.capacity())
		...
		std::cout << " Fail - Resize " <<  V1.capacity() << " vs " << V1STL.capacity() <<  std::endl;
	Comparestack(&V1, &V1STL, "Resize - 2");
	...
	if (V1.capacity() == V1STL.capacity())
		...
		std::cout << " Fail - Resize " <<  V1.capacity() << " vs " << V1STL.capacity() <<  std::endl;
	Comparestack(&V1, &V1STL, "Resize - 11");
...
void ft_relationnal_operator(void)
{
	std::stack<int> V1STL (3, 39);
	ft::stack<int> V1 (3, 39);
	std::stack<int> V2STL (3, 39);
	ft::stack<int> V2 (3, 39);
	std::stack<int> V3STL (4, 39);
	ft::stack<int> V3 (4, 39);
...
void ft_access(void)
{
	...
	Comparestack(&V1, &V1STL, "Operator - default");
...
void ft_clear(void)
{
	...
	V1STL.clear();
	V1.clear();
	Comparestack(&V1, &V1STL, "Clear - default");
}
...
void ft_swap(void)
{
	std::stack<int> V1STL (3, 39);
	ft::stack<int> V1 (3, 39);
	std::stack<int> V2STL (2,50);
	ft::stack<int> V2 (2,50);

	V1STL.swap(V2STL);
	V1.swap(V2);
	Comparestack(&V1, &V1STL, "Swap - first element");
	Comparestack(&V2, &V2STL, "Swap - second element");
}
...
void ft_insert_erase(void)
{
	...
	Comparestack(&V1, &V1STL, "Insert - one element");

	V1STL.insert(V1STL.end(), 6);
	V1.insert(V1.end(), 6);
	Comparestack(&V1, &V1STL, "Insert again - one element");

	std::stack<int> V2STL (2, 14);
	ft::stack<int> V2 (2, 14);
	V1STL.insert(V1STL.begin(), V2STL.begin(), V2STL.end());
	V1.insert(V1.begin(), V2.begin(), V2.end());
	Comparestack(&V1, &V1STL, "Insert again - one range");

	V1STL.erase(V1STL.begin());
	V1.erase(V1.begin());
	Comparestack(&V1, &V1STL, "Erase - one element");

	V1STL.erase(V1STL.begin(), V1STL.begin() + 2);
	V1.erase(V1.begin(), V1.begin() + 2);
	Comparestack(&V1, &V1STL, "Erase - one range");
}
...
void ft_push_pop_back(void)
{
	...
	V1STL.push_back(55);
	V1.push_back(55);
	Comparestack(&V1, &V1STL, "Push_back - default");

	V1STL.push_back(51);
	V1.push_back(51);
	Comparestack(&V1, &V1STL, "Push_back again - default");

	V1STL.pop_back();
	V1.pop_back();
	Comparestack(&V1, &V1STL, "Pop_back - default");

	V1STL.pop_back();
	V1.pop_back();
	V1STL.pop_back();
	V1.pop_back();
	Comparestack(&V1, &V1STL, "Pop_back again - default");
}
...
void ft_assign(void)
{
	std::stack<int> V1STL (3, 39);
	ft::stack<int> V1 (3, 39);
	std::stack<int> V2STL (3,50);
	ft::stack<int> V2 (3,50);

	V1STL.assign(++V2STL.begin(), V2STL.end());
	V1.assign(++V2.begin(), V2.end());
	Comparestack(&V1, &V1STL, "Assign - with iterator");

	V1STL.assign(7, 21);
	V1.assign(7, 21);
	Comparestack(&V1, &V1STL, "Assign - with value");

	ft::stack<std::string> VCHAR2(1,"Test");
	std::stack<std::string> VCHAR2_STL(1, "Test");
	Comparestack(&VCHAR2, &VCHAR2_STL, "Assign - with string value");

	ft::stack<std::string> VCHAR3(0,"Test");
	std::stack<std::string> VCHAR3_STL(0, "Test");
	Comparestack(&VCHAR3, &VCHAR3_STL, "Assign - with size at 0");
}

void ft_constructor(void)
{
	std::stack<int> V1_STL;
	ft::stack<int> V1;
	Comparestack(&V1, &V1_STL, "Constructor - default");

	ft::stack<char> VCHAR2(5,'A');
	std::stack<char> VCHAR2_STL(5, 'A');
	Comparestack(&VCHAR2, &VCHAR2_STL, "Constructor - default with char");

	std::stack<int> V2STL (3, 39);
	ft::stack<int> V2 (3, 39);
	Comparestack(&V2, &V2STL, "Constructor - fill");

	std::stack<int> V3STL (V2STL.begin(),V2STL.end());
	ft::stack<int> V3 (V2.begin(),V2.end());
	Comparestack(&V3, &V3STL, "Constructor - range");

	std::stack<int> V4STL (V3STL);
	ft::stack<int> V4(V3);
	Comparestack(&V4, &V4STL, "Constructor - copy");

	V1_STL = V4STL;
	V1 = V4;
	Comparestack(&V1, &V1_STL, "Constructor - operator = ");
}
